...
async function openNotaModal(nota) {
  ...
  // Cargar alumnos, asignaturas Y profesores en paralelo
  const [alumnos, asigs, profs] = await Promise.all([
    api.getAlumnos(),
    api.getAsignaturas(),
    api.getProfesores()
  ]);

  $('nota-alumno').innerHTML = '<option value="">-- Selecciona alumno --</option>' +
    alumnos.map(a => `<option value="${a.id}">${a.nombres} ${a.apellidos}</option>`).join('');

  $('nota-asig').innerHTML = '<option value="">-- Selecciona asignatura --</option>' +
    asigs.map(a => `<option value="${a.id}">${a.nombre}</option>`).join('');

  // ← CAMPO QUE FALTABA: cargar profesores en el select
  $('nota-prof').innerHTML = '<option value="">-- Sin asignar --</option>' +
  ...
  if (nota) {
    ...
    $('nota-prof').value   = nota.prof_id || '';   // ← recuperar profesor al editar
    $('nota-valor').value  = nota.valor;
    $('nota-tipo').value   = nota.tipo;
...
async function editNota(id) {
  const nota = await api.getNotas().then(ns => ns.find(n => n.id === id)).catch(() => null);
  state.editingId = id;
...
async function saveNota() {
  const data = {
    ...
    prof_id:   parseInt($('nota-prof').value) || null,  // ← enviar prof_id al backend
...
async function openAlumnoModal(alumno) {
  ...
  $('al-curso').innerHTML = cursos.map(c => `<option value="${c.id}">${c.nombre}</option>`).join('');
  if (alumno) {
...
async function openCursoModal(cu) {
  ...
  $('cu-jefe').innerHTML = '<option value="">Sin asignar</option>' +
    profs.map(p => `<option value="${p.id}">${p.nombres} ${p.apellidos}</option>`).join('');
  if (cu) {
