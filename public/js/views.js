...
async function renderDashboard() {
  ...
  $('kpi-grid').innerHTML = `
    <div class="kpi-card"><div class="kpi-label">Alumnos</div><div class="kpi-value">${alumnos.length}</div><div class="kpi-sub">${(await api.getCursos()).length} cursos activos</div><span class="kpi-badge badge-blue"><i data-lucide="users" size="11"></i> Total inscritos</span></div>
    ...
    <div class="kpi-card"><div class="kpi-label">Tasa de aprobación</div><div class="kpi-value">${tasa}%</div><div class="kpi-sub">${stats.aprobados} de ${alumnos.length} alumnos ≥ 4.0</div><span class="kpi-badge ${tasa>=80?'badge-green':'badge-orange'}">${tasa>=80?'✓ Buena':'⚠ Revisar'}</span></div>`;
...
async function renderAlumnos() {
  ...
  const [alumnos, notas] = await Promise.all([api.getAlumnos(params), api.getNotas()]);
  const promMap = {};
  notas.forEach(n => { if (!promMap[n.alumno_id]) promMap[n.alumno_id]=[]; promMap[n.alumno_id].push(n.valor); });
  const avg = arr => arr.length ? arr.reduce((a,b)=>a+b,0)/arr.length : null;
  $('alumnos-body').innerHTML = alumnos.length ? alumnos.map(a => { const p=avg(promMap[a.id]||[]); return `<tr><td><div style="display:flex;align-items:center;gap:8px"><div class="avatar">${initials(a.nombres,a.apellidos)}</div>${a.nombres} ${a.apellidos}</div></td><td style="color:var(--color-text-muted)">${a.rut||'–'}</td><td><span class="chip">${a.curso_nombre||'–'}</span></td><td style="color:var(--color-text-muted)">${a.email||'–'}</td><td><span class="pill-status ${a.estado==='Activo'?'status-activo':'status-inactivo'}">${a.estado}</span></td><td><span class="nota-badge ${p?notaColor(p):'badge-gray'}">${fmtNota(p)}</span></td><td><div class="actions-cell"><button class="btn btn-ghost btn-sm" onclick="editAlumno(${a.id})"><i data-lucide="pencil" size="12"></i></button><button class="btn btn-danger btn-sm" onclick="deleteItem('alumnos',${a.id})"><i data-lucide="trash-2" size="12"></i></button></div></td></tr>`; }).join('') : `<tr><td colspan="7"><div class="empty-state" style="padding:var(--space-10)"><div class="empty-icon"><i data-lucide="users" size="40"></i></div><h3>Sin alumnos</h3><p>Agrega el primer alumno.</p><button class="btn btn-primary" onclick="openAddModal()">Nuevo alumno</button></div></td></tr>`;
  lucide.createIcons();
}
...
async function renderCursos() {
  ...
  const [cursos, alumnos] = await Promise.all([api.getCursos(), api.getAlumnos()]);
