...
router.get('/:id', async (req, res) => {
  const db = await getDb();
  const rows = await query(db, 'SELECT * FROM profesores WHERE id = ?', [req.params.id]);
...
router.post('/', async (req, res) => {
  ...
  const db = await getDb();
  const info = await run(db, 'INSERT INTO profesores (nombres,apellidos,rut,email,especialidad,estado) VALUES (?,?,?,?,?,?)',
...
router.put('/:id', async (req, res) => {
  ...
  const db = await getDb();
  await run(db, 'UPDATE profesores SET nombres=?,apellidos=?,rut=?,email=?,especialidad=?,estado=? WHERE id=?',
    [nombres, apellidos, rut, email, especialidad, estado, req.params.id]);
  res.json({ ok: true });
});
router.delete('/:id', async (req, res) => {
  const db = await getDb();
  await run(db, 'DELETE FROM profesores WHERE id = ?', [req.params.id]);
  res.json({ ok: true });
});
